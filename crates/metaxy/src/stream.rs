...
/// before sending it to the client. The type parameter `T` carries the chunk
/// type so that the CLI can extract it for TypeScript codegen.
///
/// # Example
///
/// ```rust,ignore
...
mod tests {
    ...
    async fn send_formats_as_sse_json_event() {
        ...
        let sender: StreamSender<String> = StreamSender::new(tx);
    ...
    async fn send_formats_struct_as_sse_json_event() {
        ...
        let (tx, mut rx) = mpsc::channel(16);
        let sender: StreamSender<Token> = StreamSender::new(tx);
    ...
    async fn multiple_sends_produce_separate_events() {
        ...
        sender.send("second".to_string()).await.unwrap();
        sender.send("third".to_string()).await.unwrap();
