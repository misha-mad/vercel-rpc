...
const CLI_CMD = `cargo run --manifest-path ${join(PROJECT_ROOT, 'Cargo.toml')} -p metaxy-cli --`;
...
describe('codegen pipeline: scan → generate → validate', () => {
	...
	it('generates valid rpc-types.ts', () => {
		...
		// Header
		expect(content).toContain('auto-generated by metaxy-cli');
	...
	it('generates valid rpc-client.ts', () => {
		...
		// Imports
		expect(content).toContain('import type { Procedures,');
	...
	it('generated files pass TypeScript type-checking', () => {
		...
		const clientPath = join(TEST_OUT, 'rpc-client.ts');
		runCli(
			`generate --dir ${API_DIR} --output ${typesPath} --client-output ${clientPath} --types-import ./rpc-types`
		);

		// Run tsc --noEmit on the generated files
		const result = execSync(
			`npx tsc --noEmit --strict --moduleResolution bundler --module esnext --target esnext ${clientPath}`,
			{
				cwd: ROOT,
				encoding: 'utf-8',
				timeout: 30_000
...
describe('codegen: production files match generated output', () => {
	it('src/lib/rpc-types.ts matches fresh generation', () => {
		...
		runCli(
			`generate --dir ${API_DIR} --output ${freshTypesPath} --client-output ${freshClientPath}`
		);
