...
const VOID_QUERIES: Set<string> = new Set(["bigint_demo", "cached_time", "cached_time_private", "cached_time_stale", "cookie_demo", "dedup_demo", "init_demo", "secret", "status", "time", "types"]);
...
export interface RpcClient {
   ...
   */
  query(key: "cookie_demo"): Promise<CookieDemoResponse>;
  query(key: "cookie_demo", options: CallOptions): Promise<CookieDemoResponse>;
  /**
   * Slow endpoint (500ms) with a request counter.
   * Used to demonstrate request deduplication.
   */
  query(key: "dedup_demo"): Promise<DedupDemoResponse>;
  query(key: "dedup_demo", options: CallOptions): Promise<DedupDemoResponse>;
