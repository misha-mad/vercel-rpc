...
<div class="max-w-3xl space-y-8">
	...
	<p class="text-text-muted leading-relaxed">
		Identical in-flight queries are automatically deduplicated — only one HTTP request is made
	...
	<p class="text-text-muted leading-relaxed text-sm">
		...
		request is made. Requests are matched by procedure name + serialized input. Subsequent
	...
	<p class="text-text-muted leading-relaxed text-sm">
		...
		<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">AbortSignal</code> is wrapped
		independently — aborting one caller does not affect others sharing the same in-flight promise.
	</p>
</div>
