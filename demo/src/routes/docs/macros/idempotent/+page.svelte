...
<div class="max-w-3xl space-y-8">
	...
	<p class="text-text-muted leading-relaxed">
		...
		are <strong class="text-text-primary">never</strong> retried — even with a retry policy configured.
		Queries (GET) are always retryable.
	</p>
	...
	<p class="text-text-muted text-sm mb-2">
		Mark mutations as idempotent when repeated calls produce the same result:
		upserts, overwrites, deletes.
	</p>
	<CodeBlock html={data.highlighted['examples']} />

	<h2 class="text-xl font-semibold">Retry Behavior</h2>
	<p class="text-text-muted text-sm mb-2">
		When marked idempotent, the mutation follows the same <a href="/docs/client/retry" class="text-accent-ts hover:underline">retry policy</a> as queries.
		Without it, the mutation is never retried regardless of the retry configuration.
	</p>
