...
<div class="max-w-3xl space-y-8">
	...
	<p class="text-text-muted leading-relaxed">
		Use <code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">#[rpc_query]</code> for
		read-only operations. Wrap them with
		<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">createQuery</code> in Svelte for
	...
	<p class="text-text-muted text-sm">
		Every <code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">query()</code> call
	...
	<div class="rounded-lg border border-border bg-bg-soft p-6">
		...
		<p class="text-text-muted text-sm mb-4">
			<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono"
				>createQuery(rpc, "time")</code
			> — auto-fetches on mount, no input needed.
		</p>
		<div class="flex items-center gap-3 mb-3">
			<span class="text-sm text-text-muted"
				>Server time: <strong class="text-text-primary">
					{#if time.isLoading && !time.data}loading...{:else if time.data}{new Date(
							time.data.timestamp * 1000
						).toLocaleString()}{/if}
				</strong></span
			>
			<button
				onclick={() => time.refetch()}
				class="rounded-md bg-accent-ts px-3 py-1.5 text-sm font-medium text-white transition-opacity hover:opacity-85"
				>Refresh</button
			>
		</div>
		<button
			class="mt-3 text-xs text-text-faint hover:text-text-muted transition-colors"
			onclick={() => toggleCode('time')}
		>
	...
	<div class="rounded-lg border border-border bg-bg-soft p-6">
		...
		<p class="text-text-muted text-sm mb-4">
			<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">HealthStatus</code> enum maps
			to
			<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono"
				>"Healthy" | "Degraded" | "Down"</code
			>.
		</p>
		{#if status.data}
			<div class="rounded-md bg-bg-code p-3 text-sm space-y-1">
				<div>
					...
					<span class="text-text-primary">{status.data.name}</span>
				</div>
				<div>
					<span class="text-text-muted">Status:</span>
					<span class="text-green-400">{status.data.status}</span>
				</div>
				<div>
					<span class="text-text-muted">Uptime:</span>
					<span class="text-text-primary">{status.data.uptime_secs}s</span>
				</div>
			</div>
	...
	<div class="rounded-lg border border-border bg-bg-soft p-6">
		...
		<p class="text-text-muted text-sm mb-4">
			Demonstrates struct input and <code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono"
				>Result&lt;T, E&gt;</code
			> error handling.
		</p>
	...
	<div class="rounded-lg border border-border bg-bg-soft p-6">
		...
		<p class="text-text-muted text-sm mb-4">
			Accepts <code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">Vec&lt;f64&gt;</code
			>, uses
			<code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono">enabled</code> option to skip empty
			input.
		</p>
		<input
			type="text"
		...
		{#if stats.data}
			<div class="rounded-md bg-bg-code p-3 text-sm space-y-1">
				<div>
					...
					<span class="text-text-primary">{stats.data.count}</span>
				</div>
				<div>
					<span class="text-text-muted">Mean:</span>
					<span class="text-text-primary">{stats.data.mean.toFixed(2)}</span>
				</div>
				<div>
					<span class="text-text-muted">Min:</span>
					<span class="text-text-primary">{stats.data.min}</span>
					/ <span class="text-text-muted">Max:</span>
					<span class="text-text-primary">{stats.data.max}</span>
				</div>
			</div>
		{/if}
	</div>
	...
	<div class="rounded-lg border border-border bg-bg-soft p-6">
		...
		<p class="text-text-muted text-sm mb-4">
			Demonstrates <code class="bg-bg-code px-1.5 py-0.5 rounded text-xs font-mono"
				>serde(rename_all)</code
			>,
		...
		{#if profile.data}
			<pre
				class="bg-bg-code rounded-lg p-3 font-mono text-xs overflow-x-auto text-text-primary">{JSON.stringify(
					profile.data,
					null,
					2
				)}</pre>
